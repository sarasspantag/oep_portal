...
export class NewUploadComponent implements OnInit {
 ...
 public editorData = '<p>Hello, world!</p>';
  ...
  ngOnInit() {
    ...
    this.imgshow = false;
    this.filename = "";
    this.id = LocalStorage.getValue("newid");
    this.type = LocalStorage.getValue("newtype");
    this.form = LocalStorage.getValue("newform");
    this.sessionname = LocalStorage.getValue("newsessionname");
    this.foldername = LocalStorage.getValue("newfoldername");
        ...
        console.log(data)
          if (this.responseType == 'F') {
           ...
           {    
            ...
            const dialogManualLogin = this.dialog.open(this.alertDialog);
            LocalStorage.setValue("filename1",this.responseValue["filename"]);
