package com.oep.services.admin.communication;
...
@Controller
public class OEP_ADM_SERVC_SEND_INFO extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext arg0)
		...
		/*try {
			...
			List<Object> smsTemplatelist = jdbcTemplate.query("SELECT `ish_id`,`ish_mob_no`,`ish_sms_content` FROM `sms_history` WHERE `ish_status` = 0 "
					+ " AND ((SELECT COUNT(*) FROM `sms_history` WHERE `ish_status` = -1) = 0) "
			...
			List<Object> smsSettingsList = oep_UTIL.smsSettingsList;
		...
		try{
			...
			List<Object> emailSettingsList = oep_UTIL.emailSettingsList;
